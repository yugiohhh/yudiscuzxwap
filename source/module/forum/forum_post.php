 ...
 *
 *      $Id: forum_post.php 17704 2010-10-28 02:14:22Z liulanbo $
 */
...
/*
$_G['gp_sortid'] = isset($_G['gp_sortid']) ? intval($_G['gp_sortid']) : 0;
$_G['gp_typeid'] = isset($_G['gp_typeid']) ? intval($_G['gp_typeid']) : 0;
$_G['gp_special'] = isset($_G['gp_special']) ? intval($_G['gp_special']) : 0;
*/
...
if($sortid) {
	...
	//note 手机版中默认加载分类信息(IN_MOBILE)
	if(defined('IN_MOBILE') && !defined('TPL_DEFAULT')) {
		threadsort_optiondata($pid, $sortid, $_G['cache']['threadsort_option_'.$sortid], array());
	}
}
...
//note 应用对应的积分策略
//note $_G['setting']['creditspolicy'] 积分策略， post, reply, pm, search... 各加多少积分
//note $_G['forum']['postcredits'] $_G['forum']['replycredits'] $_G['forum']['digestcredits'] $_G['forum']['postattachcredits'] 论坛的积分策略
//note $_G['setting']['extcredits'] 各项积分的名称, 启用状况
//note $_G['setting']['creditstrans'] : 交易积分id $settings
...
if($allowposturl < 3 && $message) {
	...
	if(is_array($urllist[1])) foreach($urllist[1] as $key => $val) {
		...
		if(!iswhitelist($val)) {			
			if($allowposturl == 0) {//note 禁止发送站外连接
				showmessage('post_url_nopermission');
			} elseif($allowposturl == 1) {//note 允许发表站外连接，但是会进入审核状态
				$modnewthreads = $modnewreplies = 1;
				break;
			} elseif($allowposturl == 2) {//note 允许发表站外连接，但是不解析
